...
convertObj <- function(obj, target) {
  return(list(
    ...
    x0     = grid::convertX(obj$x0,          target), 
    x1     = grid::convertX(obj$x1,          target), 
    y0     = grid::convertY(obj$y0,          target), 
    y1     = grid::convertY(obj$y1,          target)
  ))
}
...
createCoords <- function(gt)
{
  ...
  sendFrame = do.call(rbind, sendList)
  return(sendFrame)
}
